...
def get_ppo_config_space(use_prox_curr: bool = True, use_state_novelty: bool = True) -> ConfigurationSpace:
    ...
    configspace_sb_ppo =  ConfigurationSpace({
        ...
        "ent_coef": Float("ent_coef", (0.0, 1.0), default=0.02),  # differs from stable baselines default, more exploration
    ...
    policy_layersizes = [Integer(f"policy_layer{i}_size", (1, 128), default=64) for i in range(max_layers)]
    ...
    if use_state_novelty:  # Conditions alone are not enough because ConfigSpace is quite restrictive
        ...
        rnd_activation = Categorical("rnd_activation", ("relu", "leakyrelu"), default="leakyrelu")
        ...
        rnd_layersizes = [Integer(f"rnd_layer{i}_size", (1, 128), default=default_size) for i, default_size in zip(range(max_layers), [128, 64, 32] + [32] * (max_layers - 3))]
