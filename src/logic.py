...
def pick_starting_state(value_function: Callable[[State], Number], novelty_function: Callable[[State], Number], state_candidates: Iterable[State], beta_proximal: Number, gamma_tradeoff: Number) -> State:
    ...
    state_pos = np.array([beta_proximal * value * (1 - value) for value in map(value_function, state_candidates)])
